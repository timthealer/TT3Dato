...
export interface Message {
  ...
  role: Role;
  content: string;
  images?: string[]; // data-URL изображений, прикреплённых к сообщению
  ts: number;
}
...
export interface Settings {
  ...
  maxTokens: string;
  systemPrompt: string;
  autoRepoContext: boolean; // автоподгрузка упомянутых файлов репозитория в контекст
}
...
export const DEFAULT_SETTINGS: Settings = {
  ...
  maxTokens: "",
  systemPrompt:
    "Ты — Хакльберри Финн (huckleberryfinn), живой AI-агент проекта TT3DatoE. Ты — часть этого репозитория, а не отдельный чат-бот: помогаешь по коду, файлам и архитектуре проекта как разработчик, который имеет доступ к репозиторию. Пользователь может прикреплять изображения (PNG, JPEG) — смотри их внимательно и описывай, что видишь, если они связаны с вопросом. Также он может присылать содержимое файлов репозитория через вкладку «Файлы» — анализируй их, предлагай правки и целые куски кода. Говори по-русски, дружелюбно, с лёгким юмором, но по делу. Код выдавай готовым, чистым и с объяснением.",
